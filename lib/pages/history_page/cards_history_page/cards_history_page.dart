...
class _CardsHistoryPageState extends State<CardsHistoryPage> with TickerProviderStateMixin {
  ...
  Future<void> loadCardWalletStatusFromStorage(String address) async {
    if (!_historyPageStore.isCardRefreshing) {
      await _historyPageStore
          .setCardSelectedAddress(widget.balanceStore.cards[_historyPageStore.cardHistoryIndex].address);
      await _historyPageStore.getSingleCardHistory(
        address: widget.balanceStore.cards[_historyPageStore.cardHistoryIndex].address,
      );
    }
  }
  ...
  void initState() {
    ...
    if (widget.balanceStore.cards.isNotEmpty) {
      loadCardWalletStatusFromStorage(widget.balanceStore.cards[_historyPageStore.cardHistoryIndex].address);
    }
    _historyPageStore
      ..loadLastRefreshed()
      ..cardCurrentPage = 1;
  ...
  Widget build(BuildContext context) {
    return ReactionBuilder(
      builder: (context) {
        return reaction((p0) => _historyPageStore.isCardRefreshing, (isCardRefreshing) async {
          if (!isCardRefreshing) {
            _controller
              ..stop()
              ..reset();
            await loadCardWalletStatusFromStorage(
              widget.balanceStore.cards[_historyPageStore.cardHistoryIndex].address,
            );
          }
        });
      },
      child: widget.balanceStore.cards.isEmpty
          ? Align(
          ...
          : SingleChildScrollView(
              ...
              child: Stack(
                children: [
                  Observer(
                    builder: (context) {
                      return (_historyPageStore.cardHistories[
                                  widget.balanceStore.cards[_historyPageStore.cardHistoryIndex].address] ==
                              null
                          ? const Padding(
                          ...
                          : Observer(
                              builder: (context) {
                                ...
                                if (_historyPageStore.cardsTransactions!.isEmpty) {
                                  return Align(
                                    alignment: Alignment.bottomCenter,
                                    child: Column(
                                      children: [
                                ...
                                return ListView.builder(
                                  shrinkWrap: true,
                                  controller: widget.scrollController,
                                  itemCount: _historyPageStore.cardUniqueDates!.length + 1,
                                  itemBuilder: (context, dateIndex) {
                                    if (dateIndex == _historyPageStore.cardUniqueDates?.length) {
                                      if (_historyPageStore.cardsTransactions!.length <= 19) {
                                    ...
                                    final currentDate = _historyPageStore.cardUniqueDates?[dateIndex];
                                    final transactionsOfDay = _historyPageStore.cardsTransactions
                                        ?.where(
                          ...
                          onPressed: _historyPageStore.disabledButtons[
                              ...
                              : () async {
                                  ...
                                  _historyPageStore.disabledButtons[
                                      widget.balanceStore.cards[_historyPageStore.cardHistoryIndex].address] = true;
                                  await _historyPageStore.setRefreshing(value: true);
                                  await _historyPageStore.cardRefresh(
                                    widget.balanceStore.cards[_historyPageStore.cardHistoryIndex].address,
                                  );
                                  _controller
                                    ..stop()
                                  ...
                                  _historyPageStore.disabledButtons[
                                      widget.balanceStore.cards[_historyPageStore.cardHistoryIndex].address] = false;
                                  _historyPageStore.makeActive();
                                  await recordAmplitudeEventPartTwo(
                                    RefreshClicked(address: _historyPageStore.selectedCardAddress),
                                  );
                                },
                          child: Row(
                            children: [
                              Column(
                                ...
                                children: [
                                  ...
                                  Observer(
                                    builder: (_) => Text(
                                      _historyPageStore.getLastRefreshed(_historyPageStore.selectedCardAddress),
                                      style: const TextStyle(
                                        fontFamily: FontFamily.redHatMedium,
                                        fontSize: 10,
                                        color: Color(0xFF838995),
